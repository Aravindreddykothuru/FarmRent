...
exports.getDemandPrediction = async (req, res, next) => {
  try {
    ...
    // Historical booking patterns for this region & type
    const historicalData = await EquipmentRental.aggregate([
      {
        $match: {
      ...
      {
        $match: {
          ...(district && { 'equipmentData.location.district': new RegExp(district, 'i') }),
          ...(machineType && { 'equipmentData.category': machineType }),
        },
      },
      {
        $group: {
    ...
    // Seasonal demand scoring (months 1-12)
    const seasonalScores = {
      1: 0.7, 2: 0.6, 3: 0.5,  // Rabi harvest - moderate
      4: 0.4, 5: 0.4, 6: 0.9,  // Summer low, Kharif start high
      7: 1.0, 8: 0.9, 9: 0.8,  // Kharif peak
      10: 0.9, 11: 0.8, 12: 0.7, // Post-harvest
    };
...
exports.getMachineRecommendations = async (req, res, next) => {
  try {
    ...
    // Get farmer's booking history
    const bookingHistory = await EquipmentRental.find({ renter: farmerId, status: 'completed' })
      .populate('equipment', 'category location ratings pricing')
      .limit(20);

    // Extract preferred machine types from history
    const preferredTypes = {};
    bookingHistory.forEach((b) => {
      if (b.equipment) {
        preferredTypes[b.equipment.category] = (preferredTypes[b.equipment.category] || 0) + 1;
      }
    });

    // Crop-to-machine mapping
    ...
    const machineQuery = {
      ...
      isVerified: true,
      status: 'available',
      category: { $in: recommendedTypes },
    };

    if (district) machineQuery['location.district'] = new RegExp(district, 'i');
    if (budget) machineQuery['pricing.dailyRate'] = { $lte: parseFloat(budget) };

    // Check if requested date is available
    if (date) {
      const dateObj = new Date(date);
      const bookedMachineIds = await EquipmentRental.find({
    ...
    const machines = await Equipment.find(machineQuery)
      .populate('owner', 'fullName phone ratings')
      .limit(20);
    ...
    const scoredMachines = machines.map((machine) => {
      ...
      score += (machine.ratings.average / 5) * 30;

      // History preference (0-20 pts)
      if (preferredTypes[machine.category]) {
        score += Math.min(preferredTypes[machine.category] * 5, 20);
      }

      // Price affordability (0-20 pts)
      if (budget) {
        const budgetRatio = parseFloat(budget) / machine.pricing.dailyRate;
      ...
      // Popularity (0-20 pts)
      score += Math.min(machine.totalBookings, 20);

      // Type relevance (0-10 pts)
      const typeIndex = recommendedTypes.indexOf(machine.category);
      ...
      return {
        ...
        matchReasons: [
          ...
          preferredTypes[machine.category] ? '🔁 Previously used type' : null,
    ...
    sendSuccess(res, {
      ...
      insights: {
        topMachineType: recommendedTypes[0],
        avgRatingInArea: scoredMachines.length
          ? parseFloat((scoredMachines.reduce((s, m) => s + m.machine.ratings.average, 0) / scoredMachines.length).toFixed(1))
          : 0,
        priceRange: scoredMachines.length ? {
          min: Math.min(...scoredMachines.map((m) => m.machine.pricing.dailyRate)),
          max: Math.max(...scoredMachines.map((m) => m.machine.pricing.dailyRate)),
        } : null,
      },
    });
  } catch (error) {
...
exports.getOptimalPricing = async (req, res, next) => {
  try {
    const { machineType, district, state, horsepower, condition } = req.body;

    // Get market data for similar machines
    const marketData = await Equipment.aggregate([
      {
        $match: {
    ...
    const recommendedDaily = parseFloat(
      (market.avgDailyRate * conditionMult * hpPremium).toFixed(0)
    );
    const recommendedHourly = parseFloat(
      (market.avgHourlyRate * conditionMult * hpPremium).toFixed(0)
    );
...
exports.getChurnRisk = async (req, res, next) => {
  try {
    const farmers = await User.find({ role: 'farmer', isActive: true }).select('_id fullName email lastLogin createdAt');
    const farmerIds = farmers.map((f) => f._id);

    const bookingStats = await EquipmentRental.aggregate([
      { $match: { renter: { $in: farmerIds } } },
      {
        $group: {
          _id: '$renter',
          totalBookings: { $sum: 1 },
    ...
    const riskProfiles = farmers.map((farmer) => {
      ...
      return {
        farmer: { id: farmer._id, name: farmer.fullName, email: farmer.email },
        riskScore,
        riskLevel,
