...
class PricingEngine {
    ...
    static _getSeasonalMultiplier(startDate) {
        const month = startDate.getMonth() + 1;
        if (month >= 6 && month <= 10) return parseFloat(process.env.PEAK_SEASON_MULTIPLIER) || 1.5;
        if (month >= 11 || month <= 3) return 1.2;
    ...
    static async _getDemandMultiplier(machineId) {
        try {
            ...
            if (recentBookings >= 3) return 1.0;
            return 0.95;
        } catch (err) {
